class Calculation {
    ...
    calcGradeSchriftlicheArbeit() {
        if (this.gradeGutachten1.value == 0 || this.gradeGutachten2.value == 0) {
            ...
            if (this.gradeGutachten1.value > (4 * Grade.precisionFactor) || this.gradeGutachten2.value > (4 * Grade.precisionFactor)) {
                this.gradeSchriftlicheArbeit.value = (5 * Grade.precisionFactor);
            } else {
                const result = (this.gradeGutachten1.value + this.gradeGutachten2.value) / 2;
    ...
    calcGradeVerteidigung() {
        if (this.gradeVortrag.value == 0 || this.gradeDiskussion.value == 0) {
            ...
            this.gradeVerteidigung.value = (result > (4 * Grade.precisionFactor)) ? (5 * Grade.precisionFactor) : parseInt(result);
        }
    }

    calcGradeAbschlussarbeit() {
        if (this.gradeSchriftlicheArbeit.valueShort == 0 || this.gradeVerteidigung.valueShort == 0) {
            this.gradeAbschlussarbeit.value = 0;
        } else {
            if (this.gradeSchriftlicheArbeit.valueShort > (4 * Grade.precisionFactor) || this.gradeVerteidigung.valueShort > (4 * Grade.precisionFactor)) {
                this.gradeAbschlussarbeit.value = (5 * Grade.precisionFactor);
            } else {
                let result = 0;
                if (this.variant == 1) // (2/3 + 1/3) / 3
                  result = (2 * this.gradeSchriftlicheArbeit.valueShort + this.gradeVerteidigung.valueShort) / 3;
                else if (this.variant == 2) // (3/4 + 1/4) / 4
                  result = (3 * this.gradeSchriftlicheArbeit.valueShort + this.gradeVerteidigung.valueShort) / 4;
                this.gradeAbschlussarbeit.value = parseInt(result);
            }
        }
        this.gradeAbschlussarbeitAsText = this.getGradeAsText(this.gradeAbschlussarbeit.value);
    }
